...
allvariables = [("summary"), ("summary", "description"), ("summary", "description", "labels"), ("summary", "description", "labels", "components_name"), ("summary", "description", "labels", "components_name", "priority_name"), ("summary", "description", "labels", "components_name", "priority_name", "issue_type_name")]

def execute_model_on_project(model, project_name):
    ...
    for variables in allvariables:
        ...
        dataX, dataY = preprocess_data(data, [avar for avar in allvariables[-1] if avar not in variables])

        my_model = model(variables = variables)
        ...
        with open(os.path.join(results_folder, my_model.name, project_name + ".json"), 'w') as outfile:
...
#            execute_model_on_project(model, project_name)
        import multiprocessing as mp
        with mp.Pool(8) as pool:
            pool.starmap(execute_model_on_project, zip(repeat(model), project_names))
    #            for project_name in project_names:                
